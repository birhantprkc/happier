...
// Measured from the slow-file summaries in successful canonical CI run
// 34480153195. New and ordinary specs use the conservative default above.
// This list influences scheduling only; it never selects or excludes coverage.
...
export function parseUiE2eShard(value) {
  ...
  if (!match) {
    throw new Error('UI E2E shard must use current/total syntax');
  }
...
export function partitionUiE2eSpecs({
  ...
  for (const entry of ranked) {
    ...
    for (let index = 1; index < partitions.length; index += 1) {
      ...
      if (candidate.weight < selected.weight
...
function parseArgs(argv) {
  ...
  const shardValue = shardIndex >= 0 ? argv[shardIndex + 1] : undefined;
  return parseUiE2eShard(shardValue);
}
...
function main() {
  const { current, total } = parseArgs(process.argv.slice(2));
