...
test('UI E2E shard parsing rejects malformed and out-of-range selectors', () => {
  assert.deepEqual(parseUiE2eShard('3/12'), { current: 3, total: 12 });
  assert.throws(() => parseUiE2eShard('0/12'), /between 1 and 12/);
  assert.throws(() => parseUiE2eShard('13/12'), /between 1 and 12/);
