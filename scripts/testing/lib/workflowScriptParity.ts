...
const CANONICAL_LANE_PARITY: readonly ParityDefinition[] = Object.freeze([
  {
    ...
    workflowCommands: [
      'yarn -s test:shared-packages:local',
  ...
  {
    id: 'cli:test:slow',
    rootScriptName: 'test:cli:slow',
    docsCommands: ['yarn test:cli:slow'],
    workflowCommands: ['yarn test:cli:slow', 'yarn workspace @happier-dev/cli test:slow'],
    workflowMode: 'any',
    triggerMode: 'required',
  },
  {
