...
5. **Run canonical CI once.** Use the exact coherent SHA. Default to GitHub-hosted runners. A manual Blacksmith pool is allowed only with explicit current approval and confirmed remaining included budget; it selects a backend for the same reusable workflow and must not create a copied CI graph.
...
4. use `runner_pool=github` by default; select a Blacksmith Linux pool only with explicit current approval plus confirmed included budget, and keep macOS, Windows, self-hosted, credentialed, and release-mutation lanes on their canonical runners;
