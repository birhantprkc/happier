...
4. Verify decision-material measurements from primary evidence under `.agents/skills/verify-claims`: recompute derived counts and inspect actual execution results, workload, and relevant source/environment basis. Root **Validation** governs reuse and required re-execution. Missing, stale, contradictory, or insufficient evidence requires a new measurement; a new handoff alone does not require another full suite. Independent risk-selected checks still run. Decorative counts are not evidence. This repo has seen delegates report green suites against the wrong source basis; a summary alone cannot establish success.
