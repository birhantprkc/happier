...
After a bounded direct read or push reports a page-limited backlog, the next live-tail refresh stages the latest page instead of repeatedly replaying adjacent pages. This ordinary catch-up merges cached history rather than discarding it. Detached readers still load one adjacent page on explicit forward-edge demand. This uses the source's existing page budget, not a second UI page ceiling; ordinary no-work tail probes do not raise the catching-up indicator.
