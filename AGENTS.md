...
2. Use repository skills when the requested task matches their purpose, not merely a keyword. Reuse unchanged instructions already active in context; after context loss, recover only the missing applicable instructions and task state. Do not reload a skill or its references at every lane, phase, or handoff. Use the existing routing:
...
- Reuse the existing process, run, or agent handle. Prefer completion notifications that actually reach the responsible agent; otherwise use an interruptible wait. Poll only when those mechanisms are unavailable or a concrete diagnostic question requires inspection. Do not add status checks alongside a working notification or wait mechanism.
- Within the current task and its delegates, avoid independently monitoring an operation whose executor will deliver its result. This does not require cross-session discovery, ownership negotiation, registries, or messages; do not spawn an agent solely to poll. Existing build/runtime tooling owns shared resource locks.
- Continue useful independent work while an operation is pending. An expired wait, empty output, or unchanged heartbeat leaves it pending; resume waiting on the same handle without automatically inspecting again or restarting the operation. Investigate evidence of failure, required intervention, or a material departure from expected behavior. Preserve cancellation, user steering, logs, and terminal-result verification; do not infer current health from an old observation.
- Resume an interrupted or capacity-limited lane on its existing handle when the prerequisite changes and the runtime supports it. Preserve completed findings and applicable validation. Replace only an unavailable/unresumable agent or a materially changed assignment, carrying the unresolved scope and evidence rather than full history. Do not repeat equivalent retries while the same quota or outage persists. Distinguish review-service unavailability from an evaluated denial; neither supplies missing approval or closes a required review gate.
...
- A limit, quota, timeout, retry budget, size cap, truncation threshold, or guard is product behavior. Before adding or changing one, find the canonical lifecycle/resource owner and reuse its budget or boundary. Do not invent a local default or "safety" ceiling from intuition, a nearby value, a test duration, or one observed sample; a subordinate phase must not impose a shorter competing cutoff on its containing operation without a separate contract. If no correctness, security, external/platform, storage, or measured resource boundary requires a limit, omit it. When a bound is required, name the protected resource or contract, derive the value from that boundary, make firing observable, specify recovery, and test both the boundary and valid behavior near it. Preserve useful valid data through paging, bounded projection, truncation, or explicit incompleteness when safe; reject only when the owning contract requires rejection.
...
- A useful test distinguishes the intended implementation from at least one plausible incorrect implementation. An observed meaningful RED satisfies this proof while it still exercises the relevant defect. When that evidence is missing or no longer applicable, demonstrate failure through the owning testing workflow; do not repeat a break-and-repair cycle at every handoff.
...
- At the smallest coherent integrated/package boundary, require the touched package’s typecheck/build-enforcing lane and relevant broader tests. Intermediate handoffs use focused RED/GREEN and risk-selected adjacent checks, naming any deferred check, its later owning boundary, and prerequisite. Deferred evidence cannot close its gate; final handoffs require applicable successful evidence or report the missing gate blocked.
- Reuse successful validation only after checking its actual command, scope, terminal result, and applicability to the relevant current source, dependencies, configuration, and environment. Rerun affected checks after relevant changes, failures, missing evidence, or unresolved uncertainty, not merely because work changed hands or context was compacted. Preserve independent re-execution required by risk or an explicit ship/release gate; a summary saying "passed" is not execution evidence.
