...
export const en = {
    ...
    server: {
        ...
        relayDrift: {
            ...
            bannerAccountMismatchDescription: ({ relay, account, appAccount }: { relay: string; account: string; appAccount: string }) => `This computer is connected to ${relay} as ${account}, not ${appAccount}.`,
            ...
            connectHereAction: 'Connect this computer here',
            connectedElsewhere: ({ relay }: { relay: string }) => `This computer is connected to ${relay}.`,
            connectedElsewhereAs: ({ relay, account }: { relay: string; account: string }) => `This computer is connected to ${relay} as ${account}.`,
            serviceConnectedElsewhere: ({ relay }: { relay: string }) => `The background service is connected to ${relay}.`,
...
settingsSession: {
    ...
    updates: {
        ...
        pill: {
            ...
            failed: 'Update didn’t finish',
            required: 'Update required',
            completed: 'Updated',
        },
        a11y: {
            pillAvailable: ({ count }: { count: number }) => `Updates, ${count} available`,
            pillRunning: 'Updates, updating',
            pillReady: 'Updates, restart to finish',
            pillFailed: 'Updates, an update didn’t finish',
            pillRequired: 'Updates, update required',
            pillCompleted: 'Updates, finished',
            rowOn: ({ title, where }: { title: string; where: string }) => `${title} on ${where}`,
            actionOn: ({ action, title, where }: { action: string; title: string; where: string }) => `${action} ${title} on ${where}`,
            progress: ({ percent }: { percent: number }) => `${percent} percent`,
        },
        summary: {
            checking: 'Checking for updates…',
            available: ({ count }: { count: number }) => plural({ count, singular: '1 update available', plural: `${count} updates available` }),
            ...
            unknown: 'Some versions couldn’t be checked',
            unchecked: 'No known updates',
        ...
        settingsSubtitle: {
            ...
            unknown: 'Couldn’t check for every update',
            unchecked: 'Some tools haven’t been checked',
            offline: 'Some machines are offline',
            checking: 'Checking for updates…',
        },
        action: {
            ...
            storeAndroid: 'Update in Google Play',
        ...
        row: {
            ...
            latestUnknown: 'Couldn’t check for a newer version',
            ...
            checking: 'Checking…',
            failedGeneric: 'The update didn’t finish. Try again.',
            remoteUnsupported: 'This machine can’t update its command line remotely. Update it on the machine.',
            updateNotVerified: 'The update ran, but the version didn’t change.',
            rolledBackLocal: 'The update didn’t start, so this computer went back to the previous version.',
            smokeFailed: 'The new version didn’t run on this computer, so nothing changed.',
            waitingReconnect: 'Installed, waiting for this machine to reconnect',
            lostConnection: 'Lost connection to this machine. It may still be updating.',
            couldNotStart: ({ message }: { message: string }) => `The update couldn’t start: ${message}`,
        ...
        confirmRemote: {
            title: 'Update another machine?',
            message: ({ machines }: { machines: string }) => `This updates software on ${machines}. After a command-line update its background service restarts, and running sessions there pause for a moment.`,
            sessions: ({ count }: { count: number }) => plural({ count, singular: '1 session is running there now.', plural: `${count} sessions are running there now.` }),
        },
        tray: {
    ...
    setupSurface: {
        ...
        blockedServiceConflictStatus: 'Another background service on this computer has to be sorted out first. It was left as it is — open Details to see which one.',
        blockedConsentDeclinedStatus: 'Setup stopped. The background service already on this computer was left exactly as it was.',
        blockedAccountKeptStatus: 'Setup stopped. This computer stays connected to the account it is signed in to.',
        ...
        relayMoveBodyFromTo: ({ from, to }: { from: string; to: string }) => `This computer’s background service is connected to ${from}. Move it to ${to}? It will stop answering on ${from}.`,
        accountMoveTitle: ({ account }: { account: string }) => `Switch this computer to ${account}?`,
    ...
    machine: {
        ...
        thisComputer: {
            ...
            cliOldCopyRemove: ({ path, command }: { path: string; command: string }) => `Still installed at ${path}. Remove it with ${command}`,
            cliOldCopyPath: ({ path }: { path: string }) => `Still installed at ${path}.`,
            cliChannelStable: 'Stable',
