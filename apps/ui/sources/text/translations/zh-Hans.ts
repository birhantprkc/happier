...
export const zhHans: TranslationStructure = {
  ...
  server: {
    ...
    relayDrift: {
        ...
        bannerAccountMismatchDescription: ({ relay, account, appAccount }: { relay: string; account: string; appAccount: string }) => `这台电脑以 ${account} 而非 ${appAccount} 的身份连接到 ${relay}。`,
        ...
        connectHereAction: '将这台电脑连接到这里',
        connectedElsewhere: ({ relay }: { relay: string }) => `这台电脑已连接到 ${relay}。`,
        connectedElsewhereAs: ({ relay, account }: { relay: string; account: string }) => `这台电脑以 ${account} 的身份连接到 ${relay}。`,
        serviceConnectedElsewhere: ({ relay }: { relay: string }) => `后台服务已连接到 ${relay}。`,
...
settingsSession: {
  ...
  updates: {
    ...
    row: {
      ...
      lostConnection: "与这台机器的连接已断开，它可能仍在更新。",
    ...
    confirmRemote: {
      title: "更新另一台机器？",
      message: ({ machines }: { machines: string }) => `这将更新 ${machines} 上的软件。命令行工具更新后，其后台服务会重启，那里正在运行的会话会短暂暂停。`,
      sessions: ({ count }: { count: number }) => `那里当前有 ${count} 个会话正在运行。`,
    },
    tray: {
  ...
  welcome: {
    ...
    blockedServiceConflictStatus: '需要先处理这台电脑上的另一个后台服务。它保持原样——打开“详情”可以查看是哪一个。',
    blockedConsentDeclinedStatus: '设置已停止。这台电脑上原有的后台服务保持原样。',
    blockedAccountKeptStatus: '设置已停止。这台电脑仍连接到它当前登录的账号。',
    ...
    relayMoveBodyFromTo: ({ from, to }: { from: string; to: string }) => `这台电脑的后台服务已连接到 ${from}。要转移到 ${to} 吗？它将不再在 ${from} 上响应。`,
    accountMoveTitle: ({ account }: { account: string }) => `要将这台电脑切换到 ${account} 吗？`,
  ...
  review: {
    ...
    thisComputer: {
        ...
        cliOldCopyRemove: ({ path, command }: { path: string; command: string }) => `仍安装在 ${path}。运行 ${command} 即可移除`,
        cliOldCopyPath: ({ path }: { path: string }) => `仍安装在 ${path}。`,
        cliChannelStable: '稳定版',
        cliChannelPreview: '预览版',
        cliChannelDev: '开发版',
