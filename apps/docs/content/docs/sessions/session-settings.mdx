...
In development source, app-triggered resumes and Direct session takeovers also use your current tmux settings, including the destination machine's override. This correction is not included in 0.2.12. Ordinary resume needs an updated app; Direct takeover needs both an updated app and daemon. Enabling tmux does not move an already-running session into tmux.
