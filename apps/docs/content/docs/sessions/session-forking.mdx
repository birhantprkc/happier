...
- **Provider-native when available**: Codex app-server supports native fork-at-latest plus rollback-to-point, and ACP can support fork-at-latest. OpenCode's legacy V1 server has a native fork route, but pure V2 does not; current server-mode admission therefore uses replay instead of making a dialect-ambiguous native claim.
...
- Two backends:
  - **Server-native (default)**: negotiates either the legacy V1 or pure V2 server API. V1 has native fork-at-message, while pure V2 has no fork route. Because this client cannot publish that distinction per live server, it fails closed and does not offer native server fork; use replay instead. It uses a managed local OpenCode server unless you set an explicit server URL.
