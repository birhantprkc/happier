...
### Desktop session action rail

<Callout type="info">
  In the current development build (unreleased), larger desktop layouts have a
  persistent action rail at the far right of the session. Use **Git**, **Files**,
  **Navigate** (conversation navigation), or **Agents** to open that sidebar
  surface; select the active action again to close the sidebar. **Review** opens
  directly in the details pane, independently of the sidebar. **Terminal**, when
  available, uses your preferred dock location. Git badges follow the same badge
  setting as the mobile tab bar. The rail replaces the sidebar header and its
  separate open/close button; phone navigation remains available.
</Callout>
