...
In current development builds, the inference runtime is downloaded on first use as well.
You can preinstall it from [Installables](/extending/installables), then enable the selected
model while online to cache its model files. Both must be present for offline local inference.
Disabled embeddings and custom remote embeddings do not download the local runtime.
Acquisition failures leave keyword search available; retry installation from Installables.
