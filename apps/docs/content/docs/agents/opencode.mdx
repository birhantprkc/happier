...
Auto prefers `opencode`, falls back to the retained preview command `opencode2`, and detects the
...
command with V2 selected. The same executable selection applies to managed servers, ACP sessions,
...
The server-native backend negotiates the installed server's V1 or V2 API over the same authenticated connection. Dynamic MCP registration, session diff, and manual compaction remain available when the legacy V1 surface is present; a pure-V2 server fails those operations explicitly instead of sending unsupported requests. V1 also exposes a native session-fork route, but Happier cannot safely advertise that live-only distinction before child admission, so server-mode forks use replay for both dialects.

Happier MCP tools are passed through normally when OpenCode runs through ACP. A pure V2
server-native session cannot receive per-session Happier MCP tools because the current V2 server
API has no dynamic MCP-registration route; the session remains usable and reports that capability
as unavailable instead of pretending registration succeeded.

Connected-account OAuth/request-auth is also V1-only. Happier's generated module is a callable V1
auth factory, while the pure V2 loader accepts only `{ id, effect }` or `{ id, setup }` plugins and
silently ignores incompatible modules. Use an OpenCode V1 installation or ACP for connected-account OAuth, or
use a directly materialized OpenAI/Anthropic API key with V2.
