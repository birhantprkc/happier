...
| --- | --- | --- |
| **Discover available models** | On | Lets Happier ask Anthropic which models are available to the selected Claude account. Turn it off to use Happier's built-in Claude model catalog without reading native Claude Code credentials or sending a model-catalog request. |
...
For a machine-local opt-out from dynamic Claude model discovery, set
`HAPPIER_CLAUDE_DYNAMIC_MODEL_PROBE_ENABLED=0` in the Happier CLI/daemon environment. This has the
same static-catalog result as turning off **Discover available models**, and the account setting
cannot be overridden back on by the environment variable.
