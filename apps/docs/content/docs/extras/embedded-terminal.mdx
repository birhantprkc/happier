...
<Callout type="info">
  In the current development build (unreleased), you can also open **Terminal**
  from the desktop [session action rail](/apps/app#desktop-session-action-rail).
  It uses the same dock preference as the header button.
</Callout>
