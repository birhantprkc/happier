...
version is put back and restarted, and the update reports that it was rolled back. `--to` takes an
exact published version. This works the same on macOS, Linux and Windows.
